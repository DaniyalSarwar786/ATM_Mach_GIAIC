...
interface Account {
    ...
    balance: number;
    name: string;
    transcation_history: any[];
    is_locked: boolean;
}

abstract class DataBase {
    
    protected accounts_db: Record<number, Account> = {
...
abstract class User extends DataBase{
    ...
    protected account = {} as Account;

    public get_accounts(account_number: number): Account {
        return this.accounts_db[account_number];
    }
    public get_account(): Account {
        return this.account;
    }
    public set_account(account_number:number,account: Account) {
...
class UserAccount_Authentication extends User {
    ...
    savedAccountNum: number = 0;
    private transaction : Transaction;
    private attepmts: number = 3;
    protected account = {} as Account;
...
class Transaction extends User{
    ...
    private userAccountAuthentication : UserAccount_Authentication;

    constructor(userAccountAuthentication : UserAccount_Authentication, account: Account) {
        super();
    ...
    private async transaction_logs() {
